import { Language } from '@/app/components/Resume/types';
import { API_BASE_URL } from '@/app/components/Resume'

export const createLanguage = async (profileId: number): Promise<Language> => {
  const dataRequired = {
    profile_id: profileId,
  };

  try {
    const response = await fetch(`${API_BASE_URL}/resume/api/v1/languages/`, {
      method: 'POST',
      headers: {
